...
pub fn parse_file(input: &mut &str) -> (Vec<usize>, Vec<usize>, Vec<usize>, Vec<[usize;2]>) {
    ...
    let parse_direction = one_of::<_,_,InputError<_>>(['L','R']).map(|x| if x=='R' {1} else {0} );
    let directions: Vec<usize> = terminated(repeat(0.., parse_direction), line_ending).parse_next(input).unwrap();

    let _ = line_ending::<_, InputError<_>>.parse_next(input);

    let parse_nodename = terminated(alphanumeric1::<_, InputError<_>>, take_till(0..,|c: char| c.is_ascii_control())); //and discard line
    ...
    let parse_node_to_numa = alphanumeric1::<_, InputError<_>>.map(|x| nodenums[x]);
    let parse_node_to_numb = alphanumeric1::<_, InputError<_>>.map(|x| nodenums[x]);
    let parse_node_to_numc = alphanumeric1::<_, InputError<_>>.map(|x| nodenums[x]);
